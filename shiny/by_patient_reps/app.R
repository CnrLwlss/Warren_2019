...
plotIMC.pts = function (x, y, corr = NULL, col.regions, cor.method, cex = 0.4,...) 
{
  ...
  cond <- reactive({
	pimc = dat[(dat$patrep_id==input$subject)&(dat$type=="Mean intensity"),]
  ...
  d <- reactive({
    ...
    dvals = dat[(dat$patrep_id==input$subject)&(dat$type==inpt),]
	
	zvals = dat[(dat$patrep_id==input$subject)&(dat$type=="z-score"),]
	zs = zvals$value
	...
	if(input$hichan != " "){
	  ...
	  actual_chan = allchans[agrep(input$hichan,allchans)[1]]
	  dvals$hcol = hiliteChannel(dvals, actual_chan)
	  if((input$type == "z-score")||(grepl("Ratio",input$type))) dvals$hcol[dvals$ch==mitochan] = rgb(0/255,154/255,73/255,0.25)
	  if(input$type == "z-score") dvals = dvals[!dvals$ch%in%c("Dystrophin","DNA1"),]
	}
	csig = ctrld()
	...
	dvals$regression_diff = cond()[paste(dvals$ch,dvals$cell_id)]
